...
where $\circ$ denotes the Hadamard product, $\lambda$ represents the steepness parameter of the transfer function near zero, and $\boldsymbol{w}\in\mathbb{R}^{N\times N \times I}$ is the weight tensor. The weights $\boldsymbol{w}$, modulated by the dynamic coefficients $\gamma_{i,j}\in\mathbb{R}^$, are then element-wise multiplied with the node  $\boldsymbol{A}$ via the Hadamard product. In the transfer function, each scalar $\gamma_{i,j}$ multiplies its corresponding weight matrix $\boldsymbol{w_{ij:}}$. As shown in Figure 1, the full coefficient matrix $\gamma\in\mathbb{R}^{N\times I}$ and the weight matrix $\boldsymbol{w_{ij:}}$ also undergo element-wise multiplication.
